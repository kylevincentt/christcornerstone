...
function renderContent(text: string) {
  return text.split('\n\n').map((para, i) => {
    ...
    if (para.startsWith('*') && para.endsWith('*')) {
      return (
        <p key={i} className="font-cormorant italic text-text-muted leading-relaxed mb-3" style={{ fontSize: '1.1rem' }}>
          {para.replace(/\*/g, '')}
        </p>
      );
    }
  ...
  return (
    <div style={{ paddingTop: '6rem' }}>
      ...
      >
        ...
        <p className="font-cormorant text-text-muted italic max-w-xl mx-auto leading-relaxed" style={{ fontSize: '1.2rem' }}>
          {doctrine.verse}
        </p>
      </div>
      ...
      <div className="max-w-3xl mx-auto px-8 py-16">
        ...
        >
          <blockquote className="font-cormorant text-cream-dark leading-relaxed mb-3" style={{ fontSize: '1.3rem' }}>
        ...
        <div className="prose-faith">
          {doctrine.full_content
            ...
            : <p className="text-text-light leading-relaxed" style={{ fontSize: '1.05rem' }}>{doctrine.description}</p>
          }
        </div>
